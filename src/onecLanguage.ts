...
export class BslSyntaxesBuilder {
  ...
  constructor(options?: BslSyntaxesBuilderOptions) {
    this._jsonPath = options?.jsonPath ?? "./json";
  }
  ...
  build = (): ILanguageSyntaxesDocument => {
    ...
    console.log({
      document,
      isSuccess: !!document.root && !!document.root.repository,
    });
    return document;
  };
}
